...
def send_slack_message(channel_id: str, text: str, sender_name: str, client: WebClient, parent: str = None):
    try:
...
def send_new_messages_to_slack(mbox_file: str, race: str, api_key: str, game_id: int):
    ...
    if 'activity' in resp.json():
        for msg in sorted(resp.json()['activity'], key=lambda x:x['orderid']):
            ...
            participants_email = [email_address_from_name(i.strip(), game_id) for i in msg['targetname'].split(',')]
            participants_email.append(email_address_from_name(msg['sourcename'], game_id))
            ...
            if msg_id not in message_ids:
                ...
                if msg['sourcename'] == msg['gamename'] and not msg['_replies'] and re.search('joined the game in slot|earned the award', msg['message']):
                    continue
                ...
                if len(to) > 1:
                    try:
                        to_email.remove(email_address_from_name(msg['sourcename'], game_id))
                ...
                slack_resp = send_slack_message(slack_channel_id, slack_msg, msg['sourcename'], client)
            ...
            for reply in sorted(msg['_replies'], key=lambda x:x['dateadded']):
                ...
                if reply_id not in message_ids:
                    ...
                    if len(to) > 1:
                        try:
                            to_email.remove(email_address_from_name(reply['sourcename'], game_id))
                    ...
                    slack_reply_resp = send_slack_message(slack_channel_id, slack_msg, reply['sourcename'], client, slack_thread_id)

                    if slack_reply_resp:
                        # Construct the reply and save to mailbox
                        r = email.message.EmailMessage()
                        r['From'] = email_address_from_name(reply['sourcename'], game_id)
